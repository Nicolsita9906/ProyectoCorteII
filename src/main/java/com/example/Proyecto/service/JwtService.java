...
public class JwtService {
   ...
   public String generateJwtToken(Empleado empleado) {
        return Jwts.builder()
                .setSubject(empleado.getNumeroDocuemento()) // Usamos el número de documento como identificador
                .claim("nombre", empleado.getNombreEmpleado())
                .claim("tipoDocumento", empleado.getTipoDocumento())
    ...
    public Integer getTipoDocumentoFromToken(String token) {
        try {
            Claims claims = Jwts.parserBuilder()
                    ...
                    .parseClaimsJws(token)
                    .getBody();
            return claims.get("tipoDocumento", Integer.class);
        } catch (Exception e) {
            return null;
        }
    }
